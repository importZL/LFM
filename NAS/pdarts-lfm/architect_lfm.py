...
class Architect(object):
    ...
    def _construct_model_from_theta_w(self,
                                      theta):  # and alpha - model.new() just initialises a new model (Network_w class)
    ...
    def _compute_unrolled_model_w2(self, input, target, a_i):  # network_w class
        ...
        unrolled_model = self._construct_model_from_theta_w(theta.sub(moment + dtheta.data, alpha=eta))
    ...
    def step_w2(self, input_train, target_train, input_valid, target_valid):
        w1_prime = self.model.w1
    ...
    def step_AVr(self, input_train, target_train, input_valid, target_valid):
        ...
        w2_prime, d_ailtrain_w2 = self._compute_unrolled_model_w2(input_train, target_train, a_weights)
    ...
    def step(self, input_train, target_train, input_valid, target_valid, train_arch, epoch):
        self.step_w1(input_train, target_train)
        if epoch == 10:
            self.model.copy_w1_w2()
        if epoch >= 10:
            self.step_w2(input_train, target_train, input_valid, target_valid)
            self.step_AVr(input_train, target_train, input_valid, target_valid)
