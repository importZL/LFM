...
class ResNet32(MetaModule):
    def __init__(self, num_classes, len_train, batch_size, block=BasicBlock, num_blocks=[5, 5, 5]):
    ...
    def _initialize_weight_b(self, len_train, batch_size):
        self.weight_b = Variable(1.0 * torch.ones(batch_size, len_train).cuda(), requires_grad=True)
        self._b_parameters = [
            self.weight_b,
        ]
